...
namespace ArxDev
{
    public partial class FormApplication : Form
    {
        ...
        private void buttonSearchTask_Click(object sender, EventArgs e)
        {
            try
            {
                using (var taskSelect = new Dm_TaskWork_Select())
                //using (var taskSearch = new Dm_TaskWork_Search())
                {
                    taskSelect.MaxRowCount = 30;

                    taskSelect.ID.Selected = true;
                    taskSelect.ID.Index = 0;
                    taskSelect.ID.OrderBy.Index = 0;
                    taskSelect.ID.OrderBy.Direction = EOrderByDirection.Ascending;

                    taskSelect.DESCTASK.Selected = true;
                    taskSelect.DESCTASK.Index = 1;

                    taskSelect.NOMETASK.Selected = true;
                    taskSelect.NOMETASK.Index = 2;

                    taskSelect.DATAINIZIO.Selected = true;
                    taskSelect.DATAINIZIO.Index = 3;

                    taskSelect.UTENTE.Selected = true;
                    taskSelect.UTENTE.Index = 4;

                    using (var ds = _manager.ARX_SEARCH.Dm_TaskWork_GetData_Active(taskSelect))
                    {
                        dgSearch.DataSource = ds.GetDataTable(0);
                    }
                }
