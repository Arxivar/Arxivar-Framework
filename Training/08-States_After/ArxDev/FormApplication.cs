...
namespace ArxDev
{
    public partial class FormApplication : Form
    {
        ...
        private void buttonStates_Click(object sender, EventArgs e)
        {
            ...
            {
                Cursor = Cursors.WaitCursor;
                //Mi carico la lista degli stati, gli ordini per la loro descrizione tradotta e li pongo nella DataGridLayout
                dgStates.DataSource = _manager. ...qualeServizio... . ...qualeMetodo... ("TRANSLATEDESCRIPTION");
            }
            finally
            {
                Cursor = Cursors.Default;
        ...
        private void buttonStatesByClass_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //1) Mi recupero tutte le classi documentali presenti in Arxivar Next 
                var allStates = _manager. ...qualeServizio... . ...qualeMetodo...("TRANSLATEDESCRIPTION");

                //2) Recupero informazioni della classe classe documentale (quella indicata, tramite codice, dalla finestra)
                var cls = _manager. ...qualeServizio... . ...qualeMetodo...(textBoxClassCode.Text);
                if (cls == null)
                {
                    //in caso di classe documentale non presente...
                    throw new Exception("Class not found: " + textBoxClassCode.Text);
                }

                //3) Inizio a recuperarmi gli stati documentali, tramite la classe documentale indicata precedentemente
                Dm_TipiDocumento_Stati[] statesCanSetList = _manager. ... qualeServizio... . ...qualeMetodo...(cls.SYSTEM_ID, null);                
                var filteredClasses = new List<Dm_Tabelle>();
                foreach (Dm_Tabelle dmTabelle in allStates)
                {
                    if (statesCanSetList.Any(x => x.CODICESTATO == dmTabelle.VALORE)) //quale stato ha la classe documentale ABC?
                    {
